...
SANITIZE_REPLACEMENTS = [
    (re.compile(r"品質保證部"), "质量管理部门"),
    (re.compile(r"品质保证部"), "质量管理部门"),
    (re.compile(r"電貿公司"), "相关业务部门"),
    (re.compile(r"电贸公司"), "相关业务部门"),
    (re.compile(r"品牌審款中心"), "审款部门"),
    (re.compile(r"品牌审款中心"), "审款部门"),
    (re.compile(r"投資業務發展部"), "业务发展部门"),
    (re.compile(r"投资业务发展部"), "业务发展部门"),
    (re.compile(r"本公司"), "本组织"),
    (re.compile(r"本司"), "本组织"),
    (re.compile(r"有限公司"), "某组织"),
    (re.compile(r"品誠[^\s\"')<>]*"), "匿名样例"),
    (re.compile(r"品诚[^\s\"')<>]*"), "匿名样例"),
    (re.compile(r"三宏達康[^\s\"')<>]*"), "匿名样例"),
    (re.compile(r"三宏达康[^\s\"')<>]*"), "匿名样例"),
...
def sanitize_text(text: str) -> str:
    ...
    sanitized = re.sub(r"(?<![\w\u4e00-\u9fff])公司(?!\w)", "组织", sanitized)
    return sanitized
