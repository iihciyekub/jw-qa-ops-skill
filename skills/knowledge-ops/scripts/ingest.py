...
def sanitize_extraction(result: ExtractionResult) -> ExtractionResult:
    sanitized_sections = []
    for section in result.sections:
        sanitized_sections.append(
            {
                **section,
                "heading": sanitize_text(str(section.get("heading", ""))),
                "body": sanitize_text(str(section.get("body", ""))),
            }
        )
    sanitized_tables = []
    for table in result.tables:
        rows = table.get("rows", [])
        sanitized_rows = [[sanitize_text(str(cell)) for cell in row] for row in rows]
        sanitized_tables.append({**table, "rows": sanitized_rows})
    return ExtractionResult(
        ...
        notes=sanitize_text(result.notes) if result.notes else result.notes,
    )
...
def ingest_path(skill_root: Path, source: Path, force: bool) -> List[Dict[str, Any]]:
    ...
    for candidate in discover_files(source):
        ...
        extraction = sanitize_extraction(extract_file(library_path))
